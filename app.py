...
from blog_summerizer import summerize_blog

# load_dotenv()

def process_url(url):
    try:
        # Get summary from blog_summerizer
        summary = summerize_blog(url)
        print("-"*40)
        print("Blog Summary:",summary)
        print("-" * 40)

        # convert summary to speach using Elevenlabs
        client = ElevenLabs()
...
with gr.Blocks(title="AI Podcast Generator", theme="soft") as demo:
        ...
        with gr.Row():
             summary_output=gr.Textbox(label="Status",lines=1)
        with gr.Row():
